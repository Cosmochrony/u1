...
\hypersetup{
  ...
  pdfkeywords={Weil representation; Heisenberg group; central character; operator perturbation;
...
\begin{document}
  ...
  \begin{abstract}
    The Q10 paper of the Cosmochrony programme reduces the isotropy identification
    ...
    $\theta = c/q$, itself derived from an operator perturbation estimate on the Weil
    generators.
    ...
    generators of the Weil representations at any two distinct central characters are
  ...
  \section{Introduction}
    ...
    \subsection{Main result}
      ...
      \begin{theorem}[Equidistance obstruction to the generator route]\label{thm:main}
        ...
        For all $c \neq c'$ in $(\mathbb{Z}/q\mathbb{Z})^\times$, the Weil multiplication
        generators satisfy
      ...
      It is a statement about the representations, not about~[U]: it closes one route and
  ...
  \section{Setup and Notation}\label{sec:setup}
    ...
    \subsection{Weil fingerprint energy}
      ...
      \end{equation}
      where, following O25~\cite{Beau2026a29}, $\delta r_n(c)$ is the \emph{number} of Weil
      ...
      O25 measures this on a generic three-component block
      $c_{\mathrm{block}} = (c_1,c_2,c_3)$ under a sampling protocol; it is not presented
      there as a deterministic function of a single central character.
      ...
      First, $\delta r_n(c)$ is an integer in $\{0,1,\dots,|S_n|\}$, so $\sigc(n) \in [0,1]$
    ...
    \subsection{Reduced character and BI parity}
      ...
      This reduction is unaffected by the defects recorded here and is retained at O22's own
      scope.
  ...
  \section{The Generator Route and Its Failure}\label{sec:lipschitz}
    ...
    \begin{lemma}[Weil generator distance: exact value]\label{lem:generator-perturb}
    ...
    \begin{proof}[Discussion]
      ...
      \emph{First}, the identification of $\delta r_n(c)$ with a compressed operator norm is
      not proved anywhere, and the two quantities are of different kinds: by
      Section~\ref{sec:setup}, $\delta r_n(c)$ is an integer count of newly independent
      ...
      \emph{Second}, even granting an identification, Lemma~\ref{lem:generator-perturb}
      leaves no room for the conclusion: the generator distance is $2\cos(\pi/2q)$ for
      \emph{every} pair of distinct characters, so
      $\|d\rho_{q,c} - d\rho_{q,c'}\|_{\mathrm{op}}$ admits no bound tending to zero with
      $|c-c'|/q$.
    ...
    \begin{remark}[The observable is quantised]\label{rem:uniform-n}
      ...
      Since $\delta r_n(c)$ is an integer, $\sigc(n)$ takes values in the finite grid
      $\{0, 1/|S_n|, \dots, 1\}$; two characters either give the same value or differ by at
      least $1/|S_n|$.
      A statement such as~\eqref{eq:lipschitz} with $|c-c'| = 1$ therefore forces
      $\sigma_c(n) = \sigma_{c'}(n)$ exactly as soon as $L|S_n| < q$, which is a rigidity
  ...
  \section{Consequences and Open Estimates}\label{sec:consequences}
    ...
    \begin{corollary}[Closure of Q10: withdrawn]\label{cor:q10}
      ...
      \begin{enumerate}
        ...
        Q10's chain~\eqref{eq:isotropy-chain} remains conditional on~[U], which remains open.
    ...
    \begin{remark}[What is proved and what is assumed]\label{rem:epistemic}
      ...
      are unconditional statements about Weil representations at distinct central characters,
      and retains inequality~\eqref{eq:op-stability} and the O22 parity reduction at their
      own scopes.
    ...
    \begin{remark}[The two estimates a proof would require]\label{rem:route}
      ...
      The Weil representations at distinct central characters are mutually as far apart as
